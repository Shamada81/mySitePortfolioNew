...
window.addEventListener('DOMContentLoaded', function () {
  ...
  player.init(); // const difference = new Difference('.officerold', '.officernew', '.officer__card-item');
  // difference.init();

  new _modules_difference__WEBPACK_IMPORTED_MODULE_3__["default"]('.officerold', '.officernew', '.officer__card-item').init();
  new _modules_forms__WEBPACK_IMPORTED_MODULE_4__["default"]('form.form').init();
});
...
function (_Slider) {
  ...
  _createClass(MainSlider, [{
    ...
    value: function showSlides(n) {
      ...
      if (n < 1) {
        this.slideIndex = this.slides.lendth;
      }
      ...
      this.next.addEventListener('click', function () {
        for (var i = 0; i < _this2.slides.length; i++) {
          if (_this2.slides[i].tagName !== 'BUTTON') {
            if (_this2.slides[i + 1].tagName == 'BUTTON') {
              _this2.container.append(_this2.slides[i + 1]);
            }

            var active = _this2.slides[i];

            _this2.container.append(active);
